...
async def orders():
    """
    Essa é a rota padrão de pedidos do sistema. Todas as rotas de peidos precisam de autenticação.
    """
...
async def cancelar_pedido(id_pedido: int, session: Session = Depends(pegar_sessao), usuario: Usuario = Depends(verificar_token)):
    ...
    if not pedido:
        raise HTTPException(status_code=400, detail="Peido não encontrado")
...
@order_router.post("/pedido/excluir/{id_pedido}")
async def excluir_pedido(id_pedido: int, session: Session = Depends(pegar_sessao), usuario: Usuario = Depends(verificar_token)):
    pedido = session.query(Pedido).filter(Pedido.id==id_pedido).first()
    if not pedido:
        raise HTTPException(status_code=400, detail="Peido não encontrado")
    if not usuario.admin and usuario.id != pedido.usuario:
        raise HTTPException(status_code=401, detail="Você não tem autorização para excluir o pedido")
